...
%      RasterReference - Raster reference object
...
%   axleft = centerWidth - size(legendRGB,2)/2;
%   axbottom = 30;
%   axwidth  = size(legendImg,2);
%   axheight = size(legendImg,1);
...
%   layer = refine(layers,'nasa.network','SearchField','serverurl')
...
%   demcmap(Z)
%   contourm(Z,R,[0 0],'Color','black')
%   colorbar
%   title ({'Gulf of Maine',mapRequest.Layer.LayerTitle}, ...
%      'Interpreter','none','FontWeight','bold')
%   
%   See also WebMapServer, WEBREAD, WMSFIND, WMSINFO, WMSLayer, WMSREAD
...
classdef WMSMapRequest
    ...
    properties (Access = public, Dependent = true)
        ...
        %   reference system code. Its default value is 'CRS:84' for WMS
        %   version 1.3.x and is 'EPSG:4326' for all other versions. If the
        %   default value is not found in Layer.CoordRefSysCodes then it is
    ...
    properties (GetAccess = public, SetAccess = private, Dependent)
        %RasterReference Raster reference object
        %
        %   RasterReference is a raster reference object that references
        %   the raster map to an intrinsic coordinate system.
        RasterReference
    end
    
    properties (GetAccess = public, SetAccess = private, Dependent, Hidden)
        %RasterRef Referencing matrix
        %
    ...
    methods 
        ...
        function R = get.RasterReference(self)
            R = rasterref([self.ImageHeight self.ImageWidth], ...
                self.CoordRefSysCode, strrep(self.CoordRefSysCode,'EPSG:',''), ...
                self.Latlim, self.Lonlim, self.XLim, self.YLim);
        end
        ...
        %   Example
        %   -------
        %   % Read and display the VMAP0 basic layer for the entire globe.
        ...
        %   % The rendered map has a spatial resolution of .5 degrees and
        %   % an image size of 1024-by-1024 pixels.
        ...
        %      'Rivers, Rails, City Label, and Basic Layers']})
            ...
            map.internal.assert(isscalar(self), 'map:validate:expectedScalarObject', ...
                'mapRequest');
            ...
            map.internal.assert(imageLength <= maximumSize,  'map:validate:inputTooLarge', ...
    ...
    methods (Access = private)
        ...
        % Construct the RequestURL based on properties of the class.
            ...
            map.internal.assert(~isempty(getMapRequest), 'map:wms:getMapRequestIsEmpty');
            ...
            % The background color is optional and the default color is
            % white (0xFFFFFF). Some WMS 1.3.0 servers do not set the
            % background color to white and some servers do not accept
            % setting the background color for certain image formats.
            % Therefore, set the background color if supported by the
            % server.
            if serverSupportsSettingBackgroundColor(self.ServerURL)
                % The color is expressed as a hexadecimal string 
                % with '0x' prefix.
...
map.internal.assert(~isempty(layer), 'map:wms:noLayersAfterUpdate');

% Validate non-empty LayerName properties.
layerNames = {layer.LayerName};
emptyIndex = cellfun('isempty', layerNames);
map.internal.assert(~all(emptyIndex), 'map:validate:expectedNonEmptyStrings',  ...
...
map.internal.assert(spec.containsVersion(version), 'map:wms:versionMismatch', ...
...
% projection code.
autoProjCode = 'AUTO';
map.internal.assert(~strncmp(autoProjCode, coordRefSysCode, numel(autoProjCode)), ...
    'map:wms:containsAutomaticProjectionCode');
...
if ~isempty(limitValue)
    ...
    map.internal.assert(numel(limitValue) == 2, 'map:validate:expectedTwoElementVector', ...
        limitName);
    
    map.internal.assert(limitValue(1) <= limitValue(2), 'map:validate:expectedAscendingOrder', ...
        limitName);
...
map.internal.assert(~useGeoCoords(coordRefSysCode), ...
    'map:wms:expectedMapCoordRefSysCode', name);

% Validate the limit's datatype and value.
validateLimit(value, name);
map.internal.assert(~isempty(value), 'map:wms:expectedNonEmptyValue',  ...
...
map.internal.assert(useGeoCoords(coordRefSysCode), ...
    'map:wms:expectedGeoCoordRefSysCode', name, coordRefSysCode);
...
map.internal.assert(~isempty(latlim) && all(-90 <= latlim(:)) && all(latlim(:) <= 90), ...
    'map:validate:expectedInterval', 'Latlim', '-90', '90');
...
map.internal.assert(~isempty(lonlim) && (wrappedTo180 || wrappedTo360), ...
...
if strcmpi(name, 'ImageHeight')
    map.internal.assert(value <= WMSMapRequest.MaximumHeight, ...
        'map:validate:inputTooLarge', ...
        name, num2str(WMSMapRequest.MaximumHeight));
    
    fixedHeight = getFixedHeight(layer);
    map.internal.assert(isempty(fixedHeight) || value == fixedHeight, ...
        'map:validate:expectedLengthValue', ...
        'ImageHeight', 'FixedHeight', num2str(fixedHeight));
else
    map.internal.assert(value <= WMSMapRequest.MaximumWidth, ...
        'map:validate:inputTooLarge', ...
        name, num2str(WMSMapRequest.MaximumWidth));
    
    fixedWidth = getFixedWidth(layer);
    map.internal.assert(isempty(fixedWidth) || value == fixedWidth, ...
         'map:validate:expectedLengthValue', ...
...
map.internal.assert(any(strcmpi('time', dimensionNames)), ...
    'map:validate:invalidFieldValue', ...
...
map.internal.assert(any(strcmpi('elevation', dimensionNames)), ...
    'map:validate:invalidFieldValue', ...
...
map.internal.assert(iscellstr(sampleDimension) && numel(sampleDimension) == 2, ...
    'map:validate:expectedTwoElementCellArrayOfStrings',  ...
    'SampleDimension');

map.internal.assert(any(strcmpi(sampleDimension{1}, dimensionNames)), ...
    'map:validate:invalidFieldValue', ...
...
map.internal.assert(numel(backgroundColor) == 3, ...
    'map:validate:expectedThreeElementVector', 'BackgroundColor');
end
...
map.internal.assert(isValidStyleName, 'map:validate:expectedMatchingCellSize',  ...
    'StyleName', 'Layer');

for k=1:numel(layer)
    styleNames = {layer(k).Details.Style.Name, ''};
    map.internal.assert(any(strcmpi(styleName{k}, styleNames)), ...
        'map:wms:expectedStyleNameInDetailsField', styleName{k}, ...
...
version = unique({details.Version});
if isscalar(version) && strncmp('1.3', version{1}, 3) ...
        && layerContainsCRSCode(layer, 'CRS:84')
    % Use CRS:84 as the default for version 1.3.x layers to prevent issues
    % with some servers that do not conform to the standard when using
    % EPSG:4326.
    coordRefSysCode = 'CRS:84';
    
elseif layerContainsCRSCode(layer, defaultCRS)
    % Use defaultCRS.
...
map.internal.assert(tf, 'map:wms:expectedCoordRefSysCode', coordRefSysCode);
end
...
map.internal.assert(~isempty(index), 'map:wms:coordRefSysCodeNotFound',  ...
...
if any(haveNoSubsets)
    ...
    switch lower(name)
        case 'latlim'
            for k=1:numel(layer)
                map.internal.assert(isequal(value, layer(k).Latlim), ...
                    'map:wms:geographicSubsetNotPermitted', ...
                    layer(k).LayerName, 'Latlim', ...
                    num2str(layer(k).Latlim(1)), num2str(layer(k).Latlim(2)));
            end
        case 'lonlim'
            for k=1:numel(layer)
                map.internal.assert(isequal(value, layer(k).Lonlim), ...
                    'map:wms:geographicSubsetNotPermitted', ...
        ...
        case 'xlim'
            for k=1:numel(layer)
                ...
                map.internal.assert(isequal(value, xLim), ...
                    'map:wms:spatialSubsetNotPermitted', ...
        ...
        case 'ylim'
            for k=1:numel(layer)
                ...
                map.internal.assert(isequal(value, yLim), ...
                    'map:wms:spatialSubsetNotPermitted', ...
...
if any(isFixedHeight(layer))
    ...
    for k=2:numel(layer)
        map.internal.assert(value == layer(k).Details.Attributes.FixedHeight, ...
            'map:wms:expectedEqualFieldValues', ...
...
if any(isFixedWidth(layer))
    ...
    for k=2:numel(layer)
        map.internal.assert(value == layer(k).Details.Attributes.FixedWidth, ...
            'map:wms:expectedEqualFieldValues', ...
...
tf = any(strcmp(coordRefSysCode,{WMSMapRequest.DefaultCRS,'CRS:84'}));
end
...
% Cell array of servers known to support WMS Version 1.3.0.
serversSupportingV1Point3 = { ...
    'http://vmap0.tiles.osgeo.org/wms/vmap0?', ...
    'http://giswebservices.massgis.state.ma.us/geoserver/wms?'};
if any(strcmp(serverURL, serversSupportingV1Point3))
    tf = true;
else
    ...
    if tf
        % Check the BoundingBox to see if the order is switched. As
        % indicated by the WMS Version 1.3.0 specification: "EPSG:4326
        % refers to WGS 84 geographic latitude, then longitude. That is, in
        % this CRS the X axis corresponds to latitude, and the Y axis to
        % longitude. In which case, XLim is latitude and YLim is
        % longitude."
...
function tf = serverSupportsSettingBackgroundColor(serverURL)
% Return true if the server supports setting BackgroundColor.

% Cell array of servers known to not support setting BackgroundColor.
serversNotSupportingBackgroundColor = { ...
    'http://svs.gsfc.nasa.gov/cgi-bin/wms?'};
tf = ~any(strcmp(serverURL, serversNotSupportingBackgroundColor));
end

%--------------------------------------------------------------------------

function codes = epsgGeographicCodes
% Read the geographic coordinate reference system codes from the EPSG
% gcs.csv file. For performance reasons, cache the codes.

persistent gcscodes
if isempty(gcscodes) 
    % Obtain the codes from the first column and remove
    % header name in first element (COORD_REF_SYS_CODE).
    result = map.internal.epsgread('gcs');
    gcscodes = result(:,1);  
    gcscodes(1) = []; 
end
codes = gcscodes;
...
function R = rasterref(rasterSize,epsgCode,code,latlim,lonlim,xLim,yLim)
% Construct a rasterref object given a raster size, EPSG code (a string
% with 'EPSG:' prefix), code number (a string without 'EPSG:' prefix), and
% geographic and map limits.

if useGeoCoords(epsgCode)
    % geographic coordinate reference system
    % (using either EPSG:4326 or CRS:84)
    % geographic limits are in latlim and lonlim
    R = georasterref( ...
...
elseif any(strcmp(code,epsgGeographicCodes))
    % geographic coordinate reference system
    % (not using EPSG:4326 or CRS:84)
    % geographic limits are in xLim (longitude) and yLim (latitude)
    R = georasterref( ...
        'LatitudeLimits',yLim, ...
        'LongitudeLimits',xLim, ...
        'RasterSize',rasterSize, ...
        'RasterInterpretation','cells', ...
        'ColumnsStartFrom','north');
else
    % projected coordinate reference system
    % map limits are in xLim and yLim
    R = maprasterref( ...
        'XWorldLimits',xLim, ...
        'YWorldLimits',yLim, ...
        'RasterSize',rasterSize, ...
