...
map.internal.assert(ischar(capabilitiesResponse) || isObject, ...
    'map:wms:expectedStringDocument', serverURL);
...
if (~isObject) 
    ...
    map.internal.assert(isXML, 'map:wms:expectedXML', serverURL);    
end
end
...
if ischar(capabilitiesResponse)
    ...
    map.internal.assert(~isempty(document), 'map:wms:documentIsEmpty');
...
if ~isempty(request)
    ...
    if ~isempty(map)
        ...
        % GetMap is not in capabilities document. All servers should
        % support JPEG format.
        capabilities.ImageFormats = {'image/jpeg'};
    end
end
end
