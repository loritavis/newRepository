...
%
%   See also WMSCapabilities, WMSFIND, WMSINFO, WMSLayer, WMSMapRequest, WMSREAD, WMSUPDATE
...
classdef WebMapServer <  handle
    ...
    methods       
        ...
        %   capabilities = getCapabilities(server);
            ...
            map.internal.assert(isscalar(h), 'map:validate:expectedScalarObject', 'server');
        ...
        %   % Find layers from USGS servers with the name geology in the 
        %   % server URL.
        %   usgsLayers = wmsfind('usgs.gov*geology','SearchField','serverurl');
        %
        %   % Find the layers for an individual server, 
        %   % update their properties, and append theclearm to the
        %   % updatedLayers array.
...
map.internal.assert(numel(layerURL) == 1, 'map:wms:tooManyServers', numel(layerURL));

% Validate the layer ServerURL with the input server's URL.
map.internal.assert(isequal(serverURL, layer(1).ServerURL), 'map:wms:mismatchedServerURL', ...
